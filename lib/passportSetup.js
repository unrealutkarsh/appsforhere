...
module.exports = function configurePassport(config) {
    ...
    function savePassportUserToMongo(env, req, accessToken, refreshToken, profile, done) {
        /**
    ...
    // TODO pull these configs from confit so we can handle stages and local builds
    passport.use(new PayPalStrategy({
        ...
        function (req, accessToken, refreshToken, profile, done) {
            savePassportUserToMongo(null, req, accessToken, refreshToken, profile, done);
        }
    ));
    ...
    passport.use(new PayPalStrategy({
        ...
        function (req, accessToken, refreshToken, profile, done) {
            savePassportUserToMongo('sandbox', req, accessToken, refreshToken, profile, done);
        }
    ));
    ...
    var UserInfo = function (entity, token) {
        ...
        if (parts.length === 2) {
            ...
            Token.findOne({_id: parts[1]}, function (err, token) {
                ...
                PayPalUser.findOne({_id: parts[0]}, function (ppuErr, paypaluser) {
                    ...
                    done(null, new UserInfo(paypaluser, token));
                });
            });
            ...
            Token.findOne({_id: parts[2]}, function (err, token) {
                ...
                PayPalDelegatedUser.findOne({_id: parts[1]}, function (ppuErr, delegate) {
                    ...
                    done(null, new UserInfo(delegate, token));
                });
            });
        } else {

...
function saveAccessTokenOnCompletion(req) {
    req.res.on('finish', function () {
        if (req.user) {
            req.user.token.update({_id:req.user.token._id}, {$set: {access_token: req.user.access_token}}, function () {
...
function setupForPayPalAccess(req, res, next) {
    ...
    if (req.user) {
        req.user.token.refresh_token = function (token, cb) {
            logger.verbose('Refreshing access token.');
            ensureRequires();
            assert(token.user.profileId === req.user.profileId, 'MISMATCH IN ACTIVE USER AND REFRESH TOKEN USER.');
            if (token.user instanceof PayPalDelegatedUser) {
                ...
                Token.decryptRefreshToken(req, function (decryptError, realToken) {
                    if (decryptError) {
                        req.logout();
...
module.exports = {
    configure: function (config) {
        hereapis.sandbox = new Liwp({
            appId: config.get('loginWithPayPal').sandbox.client_id,
            secret: config.get('loginWithPayPal').sandbox.secret,
            host: 'api.sandbox.paypal.com'
        });
        hereapis.live = new Liwp({
            appId: config.get('loginWithPayPal').live.client_id,
            secret: config.get('loginWithPayPal').live.secret,
            strictSSL: true
        });
    },
    hereApis: hereapis,
...
function live(method, api) {
    if (api === 'ppaas') {
        return 'https://api.paypal.com/v1/' + method;
    }
...
function sandbox(method, api) {
    if (api === 'ppaas') {
        return 'https://api.sandbox.paypal.com/v1/' + method;
    }

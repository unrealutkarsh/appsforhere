        ...
        if (err) {
            ...
            if (urlOrReportingRequestOptions._alreadyReauthed) {
                ...
                if (inflight[expressRequest.user.entity.profileId]) {
                    inflight[expressRequest.user.entity.profileId].listeners.push(doneFn);
...
function startReportingAuth(req, info, cb) {
    var listenerInfo = inflight[req.user.entity.profileId] = {listeners: [cb], id: req.user.entity.profileId};
    // Make a cheap call to identity to make sure our token is fresh
    req.user.hereApi().get({
        tokens: req.user.tokens(),
        json: true,
        url: 'https://api.paypal.com/v1/identity/openidconnect/userinfo/?schema=openid&access_token=' + req.user.tokens().access_token
    }, function (uiErr, uiRz) {
        if (uiErr) {

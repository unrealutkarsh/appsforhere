...
var Queue = require('mongodb-queue');
...
function _processNotification(notification) {
    if (notification.tries > 50) {
        logger.verbose('Notification #%s failed 50 times, pushing to dead letter.', notification.id);
    ...
    httpunch.post({
        ...
        if (sendErr) {
            ...
            q.notificationQueue.ack(n.ack, function (ackErr) {
                if (ackErr) {
                    logger.error('Failed to ack SUCCESFULLY processed notification message #%s: %s\n%s', n.id, ackErr.message, ackErr.stack);
                } else {
